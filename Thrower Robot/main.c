...
int main() {
    ...
    leds_init();
    buttons_init();
	//	sonar_init();//Comment this line if not using sonar
	
		tft_init(PIN_ON_LEFT, BLACK, WHITE, WHITE, WHITE);
		uart_init(COM3,38400);//Definition of UART PORT 3
	
		motor_init(MOTOR1, 1, 11, 1200, 1);
		motor_init(MOTOR2, 1, 11, 1200, 1);
    
		//Initialising the gpio pin for the line sensor
		gpio_init(GPIO5, GPIO_Mode_IPU);//input pin5 for the Left line Sensor
		gpio_init(GPIO6, GPIO_Mode_IPU);//input pin6 for Right line sensor
		gpio_init(GPIO7, GPIO_Mode_IPU);//input pin 7 for the counter
    ...
    while(1){
      ...
      while(get_ticks()==this_ticks);
			...
			if((this_ticks-last_ticks)>=500){//Code from here and below will only work once every 200 ms
...
//-------------------------------------------------------------------------------				
				if(current_checker!=previous_checker && previous_checker==true){
					counter++;
				}
